<a href="https://colab.research.google.com/drive/1kWh341zTG9AHWEmX7NHjN_i4LPuWzj0w?usp=sharing" target="_parent"><img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open In Colab"/></a>

# Corpus


```python
# Extraido de http://archive.ics.uci.edu/dataset/331/sentiment+labelled+sentences
corpus =   '''
...
from sklearn.naive_bayes import MultinomialNB  # Usamos MultinomialNB como ejemplo
from sklearn.metrics import accuracy_score
import numpy as np

# Se recogen las reseñas de la cadena como si se tratara de una planilla
svStringIO = StringIO(corpus)
data = pd.read_csv(svStringIO, sep='\t', header=None)
data.head()
...
```python
X = data.iloc[:,0] # extracción en X de la columna con las reseñas
y = data.iloc[:,-1] # extracción en y de la columna con los sentimientos
...
# Transformación de los vectores de palabras en matrices tf-idf
...
# Entrenamiento con Bayesiano Ingenuo
...
# Función para predecir el sentimiento de una cadena
...
```python
# Ejemplo de uso
example_text1 = "this is really bad. Awfull"
example_text2 = "this is excellent. Really beautiful"
example_text3 = "I can't believe it"
sentiment = predict_sentiment_NB(example_text1)
print(f"El sentimiento predicho para la reseña es {sentiment}")
```
...
example_text2 = "this is excellent. Really beautiful"
example_text3 = "I can't believe it"
sentiment = predict_sentiment_VM(example_text2)
print(f"El sentimiento predicho para la reseña es {sentiment}")
```
...
logistic_regression = LogisticRegression(solver='liblinear', random_state=0)  # Puedes ajustar los parámetros
...
def predict_sentiment_LR(text):
    text_vec = vectorizer.transform([text])
    text_tfidf = tfidf.transform(text_vec)
    prediction = logistic_regression.predict(text_tfidf.toarray())
    return prediction[0]
```
...
example_text2 = "this is excellent. Really beautiful"
example_text3 = "I can't believe it"
sentiment = predict_sentiment_LR(example_text2)
print(f"El sentimiento predicho para la reseña es {sentiment}")
```
